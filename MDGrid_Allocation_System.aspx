<%@ Page Title="" Language="C#" MasterPageFile="~/Grids.master" AutoEventWireup="true" CodeFile="MDGrid_Allocation_System.aspx.cs" Inherits="MDGrid_Allocation_System" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - Allocation Assignment / Systems Grid</asp:Content>
<asp:Content ID="cpHead" ContentPlaceHolderID="head" runat="Server"></asp:Content>
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">Allocation Assignment / Systems (<span id="spanRowCount" runat="server">0</span>)</asp:Content>
<asp:Content ID="cpGridSetting" ContentPlaceHolderID="cphGridSettings" runat="Server">
	<table id="tableQuickFilters" cellpadding="0" cellspacing="0" style="display: none;">
		<tr>
			<td style="padding-left: 5px;">Allocation Assignment:
				<asp:DropDownList ID="ddlQF" runat="server" TabIndex="1" AppendDataBoundItems="true">
					<asp:ListItem Text="ALL" Value="0" />
				</asp:DropDownList>
			</td>
			<td style="padding-left: 5px;">System:
				<asp:DropDownList ID="ddlQF_System" runat="server" TabIndex="2" AppendDataBoundItems="true">
...
<asp:Content ID="cpBody" ContentPlaceHolderID="ContentPlaceHolderBody" runat="Server">
	<div id="divPageContents" style="width: 100%;">
		<iti_Tools_Sharp:Grid ID="grdMD" runat="server" AllowPaging="false" PageSize="50" FirstPageText="First" LastPageText="Last" PagerMode="NumericFirstLast" AllowResize="true"
			CssClass="grid" BodyCssClass="gridBody" SelectedRowCssClass="selectedRow" HeaderCssClass="gridHeader" PagerCssClass="gridPager" FooterCssClass="gridPager" AlternatingRowColor="#dfdfdf" style="border-bottom:1px solid grey;">
	...
	<script id="jsAJAX" type="text/javascript">
		...
		function save_done(result) {
			try {
				...
				if (saved > 0) {
					msg = 'Successfully saved ' + saved + ' Allocation Assignment - Systems.';
					if (opener && opener.refreshPage) {
						opener.refreshPage();
					}
				}
				if (failed > 0) {
					msg += '\n' + 'Failed to save ' + failed + ' Allocation Assignment - Systems.';
				}
				MessageBox(msg);
	...
	<script id="jsEvents" type="text/javascript">
		...
		function resizeInnerFrame(){
		    ...
		    frame.style.height = fPageHeight + 26 + 'px';
		}
	</script>
	...
	<script id="jsInit" type="text/javascript">
		...
		function resizePage() {
			//resizePageElement('divPageContents');
		...
		function initVariables() {
			try {
				...
				if (_dcc[0] && _dcc[0].length > 0) {
					...
					_idxID = +'<%=this.DCC == null ? 0 : this.DCC["Allocation_SystemID"].Ordinal %>';
				}
			} catch (e) {
