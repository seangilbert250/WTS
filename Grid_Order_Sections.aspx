<%@ Page Language="VB" AutoEventWireup="false" CodeFile="Grid_Order_Sections.aspx.vb" Inherits="Grid_Order" Theme="Default"%>
...
<%--			<ul id="Tabs" runat="server">
				...
				<li><a href="#divSub" onclick="Tab_click('sub');">Sub Grid</a></li>
			</ul>--%>
			<div id="pageHeaderInfo" class="pageContentInfo" style="padding: 6px; height: 15px;">
			...
			<div id="pageFooter" class="pageContentHeader">
				<table style="width: 100%;">   <%--cellpadding="0" cellspacing="0" --%>
					<tr>   <%--nodrag="True"--%>
						...
						<td id="tdButtons" style="text-align: right;">
							<button id="btnClose" onclick="closeWindow(); return false;">Cancel</button>
							<button id="btnSave" onclick="saveOrder(); return false;">Update Order</button>
						</td>
					</tr>
				</table>
			</div>
		...
		<script type="text/javascript">
            ...
            if (type == 'Main') {
						...
						pTable = $('#<%=Me.tblColumnOrder.ClientID %>')[0];
					}
					else {
						...
						pTable = $('#<%=Me.tblColumnOrderSub.ClientID %>')[0];
					}
			...
			function buildColumnOrder(type) {
					...
					if (!checkBoxes[i].getAttribute('exclude')) {
						...
						if (DBNAME.length > 0) {
							...
							if (strColumns == "") {
							    ...
							    if (type == 'Sub' & checked == true)
                                    {
							        subSelectColumns = DBNAME;
                                }
							}
							else {
								strColumns += '~' + DBNAME + '|' + columnName + '|' + checked;
				...
				PageMethods.UpdateSubColumns(subSelectColumns); // Session assignment in here isn't working.
			...
			try {
				...
				$.each(defaultParentPage.filterBox.filters.find({ groups: { ParentModule: "Work", Module: "Custom" } }), function () {
					if ($(this).attr('name').toUpperCase() == 'ALLOCATION ASSIGNMENT') {
						var chkAllocationAssignment = $("#<%=Me.tblColumnOrder.ClientID %> tr:contains('Allocation Assignment')").find("input[type='checkbox']");
						var chkWorkArea = $("#<%=Me.tblColumnOrder.ClientID %> tr:contains('Work Area')").find("input[type='checkbox']");
