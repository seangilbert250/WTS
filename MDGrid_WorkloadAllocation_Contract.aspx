<%@  Title="" Language="C#" EnableViewState="false" EnableEventValidation="false" AutoEventWireup="true" CodeFile="MDGrid_WorkloadAllocation_Contract.aspx.cs" Inherits="MDGrid_WorkloadAllocation_Contract" Theme="Default" MasterPageFile="~/Grids.master" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - Workload Allocation Grid - Contract</asp:Content>
...
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">
    <table cellpadding="0" cellspacing="0" border="0" style="width: 100%">
        <tr>
            <td>Workload Allocation (<span id="spanRowCount" runat="server">0</span>)
            </td>
...
<asp:Content ID="cpBody" ContentPlaceHolderID="ContentPlaceHolderBody" runat="Server">
    ...
    <script id="jsAJAX" type="text/javascript">
        ...
        function saveContract() {
            try {
                ...
                var validation = validateContract();
                ...
                if (validation.length == 0) {
                    ...
                    if (changedRows.length == 0) {
                        ...
                        PageMethods.SaveContractChanges(<%=_workloadAllocationID%>, json, save_done, on_error);
                    }
                }
                else {
        ...
        function validateContract() {
            var validation = [];
            var $contractRows = $('.gridBody:visible, .selectedRow:visible', $('#<%=this.grdMD.ClientID %>_Grid'));
            var blnExit = false;

            if ($contractRows.length > 1) {
                validation.push('Cannot add more than one Contract');
                return validation.join('<br>');
            }

            $.each($contractRows, function () {
                if ($(this).find('select[field="Contract"]').val() == '0') validation.push('Contract cannot be empty.');
                return false;
            });
            ...
            $.each($contractRows, function () {
                ...
                var contractID = $(this).find('select[field="Contract"]').val();

                $.each($contractRows.not($(this)), function () {
                    if ($(this).find('select[field="Contract"]').val() == contractID) {
                        validation.push('Contract cannot have duplicates.');
                        blnExit = true;
                        return false;
                    }
                });
            ...
            if ($('span[field="Primary"]', $contractRows).find('input:checked').length > 1) validation.push('Only one contract can be flagged as primary.');

            if ($contractRows.length > 0) {
                if ($contractRows.length == 1) {
                    $('span[field="Primary"]', $contractRows).find('input').prop('checked', true);
                }

                if ($('span[field="Primary"]', $contractRows).find('input:checked').length == 0) {
                    validation.push('Please check primary for a contract.');
                }
            }
        ...
        function saveWorkloadAllocation() {
            try {

                var validation = validateWorkloadAllocation();
                ...
                if (validation.length == 0) {
                    ...
                    if (changedRows.length == 0) {
                        ...
                        var json = '[' + changedRows.join(",") + ']';
                        PageMethods.SaveWorkloadAllocationChanges(<%=this._contractID%>, json, save_done, on_error);
                    }
                }
                else {
                    MessageBox('Invalid entries: <br><br>' + validation);
        ...
        function validateWorkloadAllocation() {
            var validation = [];
            var $contractRows = $('.gridBody:visible, .selectedRow:visible', $('#<%=this.grdMD.ClientID %>_Grid'));
            var blnExit = false;

            if ($contractRows.length > 3) {
                validation.push('Cannot add more than three Workload Allocations');
                return validation.join('<br>');
            }

            $.each($contractRows, function () {
                if ($(this).find('select[field="WorkloadAllocation"]').val() == '0') validation.push('Workload Allocation cannot be empty.');
                return false;
            });
            ...
            $.each($contractRows, function () {
                ...
                var contractID = $(this).find('select[field="WorkloadAllocation"]').val();

                $.each($contractRows.not($(this)), function () {
                    if ($(this).find('select[field="WorkloadAllocation"]').val() == contractID) {
                        validation.push('Workload Allocation cannot have duplicates.');
                        blnExit = true;
                        return false;
                    }
                });
            ...
            var programMgmt = 0, deployment = 0, production = 0;
            $contractRows.each(function () {
                var waType = $(this).find('select[id*="ddlWorkloadAllocation"] option:selected').text().split(':')[0];
                switch (waType) {
                    case 'Program MGMT':
                        programMgmt++;
                        break;
            ...
            if (programMgmt > 1 || deployment > 1 || production > 1) {
                validation.push('Workload Allocation cannot have duplicates of the same type.');
            }
    ...
    <script id="jsEvents" type="text/javascript">
        ...
        function buttonNew_click() {
            ...
            if ($contractRows.length >= 3) {
                MessageBox('Cannot add more than three Workload Allocations');
            } else {
                ...
                $(nRow.cells).each(function (i, td) {
                    if ($(td).find('input:text').length > 0) {
                        ...
                        if (_dcc[0][i].ColumnName.toUpperCase() == 'PRIMARY') $(td).find('span').attr('field', 'Primary');
                    }
        ...
        function buttonSave_click() {
            if (<%=this._workloadAllocationID%> > 0) {
                saveContract();
            } else if (<%=this._contractID%> > 0) {
                ...
                if (direction.toUpperCase() == "SHOW") {
                    ...
                    $(sender).closest('tr').each(function () {
                        ...
                        td = $('td:eq(<%=(this.DCC == null 
							|| !this.DCC.Contains("WorkloadAllocation")) ? 0 : this.DCC["WorkloadAllocation"].Ordinal %>)', row)[0];
                        loadChildGrid(td, id);
                    });
                }
                else {
    ...
    <script id="jsInit" type="text/javascript">
        ...
        function initVariables() {
            try {
                ...
                _idxDelete = parseInt('<%=this.DCC["Y"].Ordinal %>');
