<%@ Page Title="" Language="C#" MasterPageFile="~/EditTabs.master" AutoEventWireup="true" CodeFile="AOR_Meeting_Instance_Attachments.aspx.cs" Inherits="AOR_Meeting_Instance_Attachments" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Meeting Instance Attachments</asp:Content>
	...
	<table cellpadding="0" cellspacing="0" style="float: right;">
		<tr>
            <td style="padding-right:20px">
                <input type="checkbox" id="cbShowAllAttachments">Show all attachments
            </td>
			<td>
...
<asp:Content ID="cpBody" ContentPlaceHolderID="ContentPlaceHolderBody" runat="Server">
	...
	<script type="text/javascript">
		var _pageUrls;
		var _canEdit = false;
		var _selectedId = 0, _meetingInstanceID = 0;
		var _idxID = 0, _idxFileName = 0, _idxAttachmentType = 0, _idxTitle = 0;

        function refreshGrid() {
            var showAll = $('#cbShowAllAttachments').is(':checked');

            var nURL = document.location.href;
            nURL = editQueryStringValue(nURL, 'ShowAll', showAll);

			document.location.href = 'Loading.aspx?Page=' + nURL;
		}
		...
		function btnDelete_click() {
			if (_selectedId == null || _selectedId == 0) {
				...
				if (confirm('Are you sure you want to delete: ' + selectedFileName + '?')) {
                    PageMethods.DeleteAttachment(_meetingInstanceID, _selectedId
		...
		function deleteAttachment_Done(result) {
			...
			try {
				...
				if (deleted) {
					successMessage('Attachment has been deleted.');
					refreshGrid();
				}
				else {
        ...
        $(document).ready(function () {
            $('#cbShowAllAttachments').prop('checked', <%=ShowAll.ToString().ToLower()%>);

			$('#imgReport').hide();
			$('#imgSort').hide();
			$('#imgExport').hide();
            $('#imgRefresh').click(function () { refreshGrid(); });
            $('#cbShowAllAttachments').click(function () { refreshGrid(); });
			...
			try {
				...
				_meetingInstanceID = parseInt('<%=AORMeetingInstanceID %>');
