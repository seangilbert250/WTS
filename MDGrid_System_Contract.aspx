<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MDGrid_System_Contract.aspx.cs" Inherits="MDGrid_System_Contract" MasterPageFile="~/Grids.master" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - System / Contracts Grid</asp:Content>
<asp:Content ID="cpHead" ContentPlaceHolderID="head" runat="Server"></asp:Content>
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">System / Contracts (<span id="spanRowCount" runat="server">0</span>)</asp:Content>
		    ...
		    try {
		        ...
		        if (validation.length == 0) {
		            ...
		            if (changedRows.length == 0) {
		                ...
		                PageMethods.SaveChanges('<%=_qfSystemID %>', json, save_done, on_error);
		            }
		        }
		        else {
		...
		function buttonNew_click() {
			...
			$(nRow.cells).each(function(i, td){
				if($(td).find('input:text').length > 0) {
					...
					if (_dcc[0][i].ColumnName.toUpperCase() == 'CONTRACT') $(td).find('select').attr('field', 'Contract');
				}
				else{
					$(td).html('&nbsp;');
				}
		...
		function initVariables() {
			try {
				...
				if (_dcc[0] && _dcc[0].length > 0) {
					...
					_idxID = +'<%=this.DCC == null ? 0 : this.DCC["WTS_SYSTEM_CONTRACTID"].Ordinal %>';
				}
			} catch (e) {
