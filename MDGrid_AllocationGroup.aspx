<%@ Title="" Language="C#" EnableViewState="false" EnableEventValidation="false" AutoEventWireup="true" CodeFile="MDGrid_AllocationGroup.aspx.cs" Inherits="MDGrid_AllocationGroup" Theme="Default" MasterPageFile="~/Grids.master" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - Allocation Group Grid</asp:Content>
...
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">
    <table cellpadding="0" cellspacing="0" border="0" style="width: 100%">
        <tr>
            <td>
             Allocation Group (<span id="spanRowCount" runat="server">0</span>)
            </td>
...
<asp:Content ID="cpPageContentInfo" ContentPlaceHolderID="cphPageContentInfo" runat="Server">
	<table cellpadding="0" cellspacing="0" style="float: right; padding-right: 2px;">
		<tr>
			<td>
                <input type="hidden" id="buttonAssignTo" value="Assign To" disabled="disabled"/>
				<input type="button" id="buttonNew" value="Add Group" disabled="disabled" />
...
<asp:Content ID="cpBody" ContentPlaceHolderID="ContentPlaceHolderBody" runat="Server">
	...
	<script id="jsEvents" type="text/javascript">
		        ...
		        if (direction.toUpperCase() == "SHOW") {
		            ...
		            $(sender).closest('tr').each(function () {
		                ...
		                td = $('td:eq(<%=(this.DCC == null 
							|| !this.DCC.Contains("ALLOCATIONGROUP")) ? 0 : this.DCC["ALLOCATIONGROUP"].Ordinal %>)', row)[0];
						loadChildGrid(td, id);
					});
                }
                else {
        ...
        function loadChildGrid(td, id) {
            ...
            url += 'MDGrid_AllocationGroup_Assignment.aspx';
            url += '?AllocationGroupID=' + id;
        ...
        function buttonAssignTo_Click(){
            if(_selectedId){
                var url = 'Loading.aspx?Page=';
                url += 'MDGrid_AllocationGroup_AssignChildren.aspx';
                url += '?AllocationGroupID=' + _selectedId;

                var nPopup = popupManager.AddPopupWindow("AppendTo", "Allocation Group - Child Grid", url, 800, 1400, "PopupWindow", this.self);
                if (nPopup) {
                    nPopup.Open();
                }
            }
            else{
                MessageBox('Please select a row which you would like to "AssignTo".');   
            }
