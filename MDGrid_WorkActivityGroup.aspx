<%@ Page Title="" Language="C#" MasterPageFile="~/Grids.master" AutoEventWireup="true" CodeFile="MDGrid_WorkActivityGroup.aspx.cs" Inherits="MDGrid_WorkActivityGroup" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - Work Activity Group Grid</asp:Content>
...
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">
        ...
        <tr>
            <td>
                Work Activity Group (<span id="spanRowCount" runat="server">0</span>)
            </td>
...
<asp:Content ID="cpGridSetting" ContentPlaceHolderID="cphGridSettings" runat="Server">
	<table id="tableQuickFilters" cellpadding="0" cellspacing="0" style="display:none;">
		<tr>
			<td style="padding-left: 5px;">Work Activity Group:
		...
		function initVariables() {
			try {
				...
				if (_dcc[0] && _dcc[0].length > 0) {
					...
					_idxID = parseInt('<%=this.DCC["WorkActivityGroupID"].Ordinal %>');
					_idxName = parseInt('<%=this.DCC["WorkActivityGroup"].Ordinal %>');
