<%@ Page Title="" Language="C#" MasterPageFile="~/Grids.master" AutoEventWireup="true" CodeFile="MDGrid_WorkArea_System.aspx.cs" Inherits="MDGrid_WorkArea_System" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - Work Area / Systems Grid</asp:Content>
<asp:Content ID="cpHead" ContentPlaceHolderID="head" runat="Server"></asp:Content>
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">Work Area / Systems (<span id="spanRowCount" runat="server">0</span>)</asp:Content>
<asp:Content ID="cpGridSetting" ContentPlaceHolderID="cphGridSettings" runat="Server">
	<table id="tableQuickFilters" cellpadding="0" cellspacing="0" style="display:none;">
		<tr>
			<td style="padding-left: 5px;">Work Area:
	...
	<script id="jsAJAX" type="text/javascript">
			...
			try {
                ...
                $('.gridBody, .selectedRow', $('#<%=this.grdMD.ClientID%>_Grid')).each(function (i, row) {
					...
					if (_dcc[0].length > 0 && $(this)[0].hasAttribute('fieldChanged')) {
						...
						if (changed) {
                            for (var i = 0; i <= _dcc[0].length - 1; i++) {
                                if ((_dcc[0][i].ColumnName == "WTS_SYSTEM") && (GetColumnValue(row, i) == "0")) {
                                    saveContinue = false;
                                    return;
                                }
                                else {
                ...
                if (!saveContinue) {
                    ShowDimmer(false);
                    MessageBox('System(Task) cannot be left blank when saving.');
                }
				    ...
				    var cvVal = getParameterByName("ChildView");
					PageMethods.SaveChanges(json, cvVal, save_done, on_error);
                }
		...
		function save_done(result) {
			try {
				...
				if (saved > 0) {
				    msg = 'Successfully saved ' + saved;
				    if(cvVal == "0"){msg += ' Work Area - Systems.';}
				    else{msg += ' Allocation.';}
		...
		function deleteItem(itemId, item) {
			try {
				...
				var cvVal = getParameterByName("ChildView");
				PageMethods.DeleteItem(parseInt(itemId), item, cvVal, deleteItem_done, on_error);
		...
		function initVariables() {
			try {
				...
				if (_dcc[0] && _dcc[0].length > 0) {
					...
					_idxID = +'<%=this.DCC == null ? 0 : this.DCC["WorkArea_SystemID"].Ordinal %>';
				}
			} catch (e) {
