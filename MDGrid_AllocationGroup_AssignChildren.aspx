<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MDGrid_AllocationGroup_AssignChildren.aspx.cs" Inherits="MDGrid_AllocationGroup_AssignChildren" MasterPageFile="~/Grids.master" Theme="Default"%>
...
<asp:Content ID="cpBody" ContentPlaceHolderID="ContentPlaceHolderBody" runat="Server">
	<div id="divPageContents" style="width: 100%;">
		<iti_Tools_Sharp:Grid ID="grdMD" runat="server" AllowPaging="true" PageSize="25" FirstPageText="First" LastPageText="Last" PagerMode="NumericFirstLast" AllowResize="true"
	...
	<script id="jsAJAX" type="text/javascript">
	    ...
	    function save() {
	        try {
	            ...
	            var parentID = '<%=_qfAllocationCategoryID%>';
	            $('.gridBody, .selectedRow', $('#<%=this.grdMD.ClientID%>_Grid')).each(function (i, row) {
				    ...
				    if (_dcc[0].length > 0) {
				        ...
				        if (changed) {
				            for (var i = 0; i <= _dcc[0].length - 1; i++) {
				                changedRow.push('"' + _dcc[0][i].ColumnName + '":"' + (_dcc[0][i].ColumnName == 'X' ? encodeURIComponent(GetColumnValue(row, i)) : GetColumnValue(row, i)) + '"');
				            }
