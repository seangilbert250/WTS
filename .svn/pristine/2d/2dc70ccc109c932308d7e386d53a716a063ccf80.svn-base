<%@ Page Title="" Language="C#" MasterPageFile="~/Grids.master" AutoEventWireup="true" CodeFile="MDGrid_WorkloadGroup.aspx.cs" Inherits="MDGrid_WorkloadGroup" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - Functionality / Workload Group Grid</asp:Content>
...
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">
        ...
        <tr>
            <td>
                Functionality / Workload Group (<span id="spanRowCount" runat="server">0</span>)
            </td>
		...
		function initVariables() {
			try {
				...
				if (_dcc[0] && _dcc[0].length > 0) {
					...
					_idxID = +'<%=this.DCC == null ? 0 : this.DCC["WorkloadGroupID"].Ordinal %>';
				}
			} catch (e) {
