<%@ Page Title="" Language="C#" MasterPageFile="~/Grids.master" AutoEventWireup="true" CodeFile="MDGrid_EffortArea.aspx.cs" Inherits="MDGrid_EffortArea" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - Area of Effort</asp:Content>
...
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">
        ...
        <tr>
            <td>
             Area of Effort (<span id="spanRowCount" runat="server">0</span>)
            </td>
				...
				if (direction.toUpperCase() == "SHOW") {
					...
					$(sender).closest('tr').each(function () {
						...
						td = $('td:eq(<%=(this.DCC == null 
							|| !this.DCC.Contains("EffortArea")) ? 0 : this.DCC["EffortArea"].Ordinal %>)', row)[0];
						loadChildGrid(td, id);
					});
				}
				else {
		...
		function initVariables() {
			try {
				...
				if (_dcc[0] && _dcc[0].length > 0) {
					...
					_idxID = +'<%=this.DCC == null ? 0 : this.DCC["EffortAreaID"].Ordinal %>';
				}
			} catch (e) {
