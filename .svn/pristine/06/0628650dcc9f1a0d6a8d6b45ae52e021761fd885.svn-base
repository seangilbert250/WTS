<%@ Page Title="" Language="C#" MasterPageFile="~/Grids.master" AutoEventWireup="true" CodeFile="MDGrid_Resource_System.aspx.cs" Inherits="MDGrid_Resource_System" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - Resource / Systems Grid</asp:Content>
<asp:Content ID="cpHead" ContentPlaceHolderID="head" runat="Server"></asp:Content>
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">Resource / Systems (<span id="spanRowCount" runat="server">0</span>)</asp:Content>
<asp:Content ID="cpPageContentInfo" ContentPlaceHolderID="cphPageContentInfo" runat="Server">
	<table cellpadding="0" cellspacing="0" style="float: right; padding-right: 2px;">
		<tr>
			<td>
				<input type="button" id="buttonNew" value="Add" disabled="disabled" style="display:none;"/>
				<input type="button" id="buttonSave" value="Save" disabled="disabled" style="display:none;"/>
			</td>
		</tr>
	</table>
</asp:Content>
		...
		function save() {
			try {
				...
				if (changedRows.length == 0) {
                    ...
                    PageMethods.SaveChanges('', '<%=_qfProductVersionID %>', json, cvVal, save_done, on_error);
				}
			} catch (e) {
				ShowDimmer(false);
		...
		function save_done(result) {
			try {
				...
				if (saved > 0) {
				    ...
				    if(cvVal == "0"){msg += ' Resource - Systems.';}
				    else{msg += ' Allocation.';}
		...
		function buttonNew_click() {
			...
			$(nRow.cells).each(function(i, td){
				if($(td).find('input:text').length > 0) {
                    ...
                    if ($(td).find('select').val() === "0") $(td).find('select').prop('disabled', false);
