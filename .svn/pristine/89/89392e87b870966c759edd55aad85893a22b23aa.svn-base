<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MDGrid_System_Resource.aspx.cs" Inherits="MDGrid_System_Resource" MasterPageFile="~/Grids.master" Theme="Default" %>
		...
		function save() {
			try {
				...
				if (changedRows.length == 0) {
					...
					PageMethods.SaveChanges('<%=_qfSystemID %>', '<%=_qfReleaseID %>', json, save_done, on_error);
				}
			} catch (e) {
				ShowDimmer(false);
		...
		function initVariables() {
			try {
				...
				if (_dcc[0] && _dcc[0].length > 0) {
					...
					_idxID = +'<%=this.DCC == null ? 0 : this.DCC["WTS_SYSTEM_RESOURCEID"].Ordinal %>';
				}
			} catch (e) {
