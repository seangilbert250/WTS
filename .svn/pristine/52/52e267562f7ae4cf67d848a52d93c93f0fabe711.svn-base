<%@ Page Title="" Language="C#" MasterPageFile="~/Grids.master" AutoEventWireup="true" CodeFile="MDGrid_Priority.aspx.cs" Inherits="MDGrid_Priority" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - Priority Grid</asp:Content>
...
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">
        ...
        <tr>
            <td>
             Priority (<span id="spanRowCount" runat="server">0</span>)
            </td>
...
<asp:Content ID="cpGridSetting" ContentPlaceHolderID="cphGridSettings" runat="Server">
	<table id="tableQuickFilters" cellpadding="0" cellspacing="0">
		<tr>
			<td style="padding-left: 5px;">Priority:
...
<asp:Content ID="cpBody" ContentPlaceHolderID="ContentPlaceHolderBody" runat="Server">
	...
	<script id="jsEvents" type="text/javascript">
		...
		function refreshPage() {
			...
			qs = editQueryStringValue(qs, 'PriorityTypeID', $('#<%=this.ddlQF.ClientID %> option:selected').val());
		...
		function lbEdit_click(itemID) {
			if (parent.ShowFrameForEdit) {
				parent.ShowFrameForEdit(_pageURLs.MasterData.MDType.Priority, false, itemID, true);
			}
			else {
				var title = '', url = '';
				var h = 700, w = 1000;

				title = 'Priority - [' + itemID + ']';
				url = _pageURLs.MasterData.Edit.Priority
					+ '?itemID=' + itemID;

				//open in a popup
				var openPopup = popupManager.AddPopupWindow('Priority', title, 'Loading.aspx?Page=' + url, h, w, 'PopupWindow', this);
				if (openPopup) {
					openPopup.Open();
				}
			}
		...
		function initVariables() {
			try {
				...
				if (_dcc[0] && _dcc[0].length > 0) {
					...
					_idxID = parseInt('<%=this.DCC["PriorityID"].Ordinal %>');
					_idxName = parseInt('<%=this.DCC["Priority"].Ordinal %>');
