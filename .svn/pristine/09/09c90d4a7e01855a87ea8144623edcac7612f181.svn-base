...
public partial class MDGrid_WorkType_Phase : System.Web.UI.Page
{
	protected DataTable _dtWorkType = null;
	protected DataTable _dtPhase = null;
	...
	private void readQueryString()
	{
		...
		if (Request.QueryString["PhaseID"] != null
			&& !string.IsNullOrWhiteSpace(Request.QueryString["PhaseID"].ToString()))
		{
			int.TryParse(Server.UrlDecode(Request.QueryString["PhaseID"].ToString()), out this._qfPhaseID);
		}
			...
			foreach (DataColumn gridColumn in dt.Columns)
			{
				...
				switch (gridColumn.ColumnName)
				{
					case "X":
						...
						blnVisible = true;
						break;
					case "WorkType_PHASEID":
						displayName = "WorkType_PHASEID";
						blnVisible = false;
						blnSortable = false;
						break;
					case "WorkTypeID":
					...
					case "WorkType_SORT_ORDER":
						...
						break;
					case "PDDTDR_PHASEID":
						displayName = "PDDTDR_PHASEID";
						blnVisible = false;
						blnSortable = false;
						break;
					case "PDDTDR_PHASE":
						displayName = "PD2TDR Phase";
						blnVisible = true;
						blnSortable = true;
						break;
					case "Phase_SORT_ORDER":
						displayName = "Phase_SORT_ORDER";
						blnVisible = false;
						blnSortable = true;
						break;
					case "DESCRIPTION":
	...
	void grdMD_GridRowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		//add edit link
		string itemId = row.Cells[DCC.IndexOf("WorkType_PHASEID")].Text.Trim();
		if (itemId == "0")
		{
		...
		if (CanView)
		{
			...
			selectedId = row.Cells[DCC.IndexOf("PDDTDR_PHASEID")].Text;
			selectedText = row.Cells[DCC.IndexOf("PDDTDR_PHASE")].Text;
			if (itemId == "0" && _qfPhaseID != 0)
			{
				selectedId = _qfPhaseID.ToString();
				selectedText = string.Empty;
			}
			ddl = WTSUtility.CreateGridDropdownList(_dtPhase, "Phase", "PDDTDR_PHASE", "PDDTDR_PHASEID", itemId, selectedId, selectedText, null);
			ddl.Enabled = (_qfPhaseID == 0);
			row.Cells[DCC.IndexOf("PDDTDR_PHASE")].Controls.Add(ddl);
		...
		{
			row.Cells[DCC["X"].Ordinal].Controls.Add(WTSUtility.CreateGridDeleteButton(itemId, row.Cells[DCC.IndexOf("PDDTDR_PHASE")].Text.Trim() + " - " + row.Cells[DCC.IndexOf("WorkType")].Text.Trim()));
		}
	}
	...
	void grdMD_GridPageIndexChanging(object sender, GridViewPageEventArgs e)
	{
		...
		if (HttpContext.Current.Session["dtMD_WorkType_Phase"] == null)
		{
			loadGridData();
		}
		else
		{
			grdMD.DataSource = (DataTable)HttpContext.Current.Session["dtMD_WorkType_Phase"];
		}
	}
	...
	void formatColumnDisplay(ref GridViewRow row)
	{
		...
		row.Cells[DCC.IndexOf("PDDTDR_PHASE")].Style["width"] = "150px";
	...
	public static string SaveChanges(string rows)
	{
		...
		try
		{
			...
			int id = 0, phaseId = 0, workTypeID = 0, archive = 0;
			...
			foreach (DataRow dr in dtjson.Rows)
			{
				...
				int.TryParse(dr["WorkType_PhaseID"].ToString(), out id);
				int.TryParse(dr["PDDTDR_PHASE"].ToString(), out phaseId);
				...
				if (phaseId == 0 || workTypeID == 0)
				{
					tempMsg = "You must specify a PD2TDR Phase and a Work Activity.";
					saved = false;
				}
				else
				{
					...
					{
						exists = false;
						saved = MasterData.WorkType_Phase_Add(phaseID: phaseId, workTypeID: workTypeID, description: description, exists: out exists, newID: out id, errorMsg: out tempMsg);
						if (exists)
						{
							saved = false;
							tempMsg = string.Format("{0}{1}{2}", tempMsg, tempMsg.Length > 0 ? Environment.NewLine : "", "Cannot add duplicate Work Activity - PD2TDR Phase record.");
						}
					}
					else
					{
						saved = MasterData.WorkType_Phase_Update(workTypePhaseID: id, phaseID: phaseId, workTypeID: workTypeID, description: description, archive: (archive == 1), errorMsg: out tempMsg);
					}
				}
	...
	public static string DeleteItem(int itemId, string item)
	{
		...
		try
		{
			...
			{
				deleted = MasterData.WorkType_Phase_Delete(itemId, out exists, out hasDependencies, out archived, out errorMsg);
