...
public partial class MDGrid_RQMT_Type : System.Web.UI.Page
{
    ...
    private void exportExcel(DataTable dt)
    {
        ...
        String strName = "Master Grid - Requirement Type";
        ...
        {
            if (parentRow.Field<int>("RQMTTypeID") != 0)
            {
    ...
    private static void formatParent(ref DataTable dt)
    {
        ...
        dt.Columns["RQMTType"].ColumnName = "Requirement Type";
    }
    private void loadGridData()
    {
        DataTable dt;

        if (_refreshData || Session["dtMD_RQMT_Type"] == null)
        {
            dt = MasterData.RQMT_TypeList_Get(includeArchive: true);
            HttpContext.Current.Session["dtMD_RQMT_Type"] = dt;
        }
        else
        {
            dt = (DataTable)HttpContext.Current.Session["dtMD_RQMT_Type"];
        }
    ...
    void grdMD_GridRowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        //add edit link
        string itemId = row.Cells[DCC.IndexOf("RQMTTypeID")].Text.Trim();
        if (itemId == "0")
        {
        ...
        if (CanView)
        {
            row.Cells[DCC.IndexOf("RQMTType")].Controls.Add(WTSUtility.CreateGridTextBox("RQMTType", itemId, Server.HtmlDecode(row.Cells[DCC.IndexOf("RQMTType")].Text.Replace("&nbsp;", " ").Trim())));
            ...
            bool internalType = row.Cells[DCC.IndexOf("Internal")].Text == "1";
            row.Cells[DCC.IndexOf("Internal")].Controls.Clear();
            row.Cells[DCC.IndexOf("Internal")].Controls.Add(WTSUtility.CreateGridCheckBox("Internal", itemId, internalType));
        }
        ...
        row.Cells[DCC["X"].Ordinal].Controls.Add(WTSUtility.CreateGridDeleteButton(itemId, row.Cells[DCC.IndexOf("RQMTType")].Text.Trim()));
        //}
    ...
    void grdMD_GridPageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        ...
        if (HttpContext.Current.Session["dtMD_RQMT_Type"] == null)
        {
            loadGridData();
        }
        else
        {
            grdMD.DataSource = (DataTable)HttpContext.Current.Session["dtMD_RQMT_Type"];
        }
    }
    ...
    void formatColumnDisplay(ref GridViewRow row)
    {
        ...
        row.Cells[DCC.IndexOf("RQMTType")].Style["width"] = "100px";
        ...
        row.Cells[DCC.IndexOf("Internal")].Style["width"] = "35px";
        row.Cells[DCC.IndexOf("Internal")].Style["text-align"] = "center";
    }
    ...
    public static string SaveChanges(string rows)
    {
        ...
        bool exists = false, saved = false, duplicate = false, internalType = false;
        ...
        try
        {
            ...
            string RQMTType = string.Empty, description = string.Empty;
            ...
            foreach (DataRow dr in dtjson.Rows)
            {
                ...
                int.TryParse(dr["RQMTTypeID"].ToString(), out id);
                RQMTType = server.UrlDecode(dr["RQMTType"].ToString());
                ...
                internalType = dr["Internal"].ToString() == "1";

                if (string.IsNullOrWhiteSpace(RQMTType))
                {
                    tempMsg = "You must specify a value for the Requirement Type.";
                    saved = false;
                }
                else
                {
                    ...
                    {
                        exists = false;
                        saved = MasterData.RQMTType_Add(RQMTType: RQMTType, description: description, sort: sortOrder, archive: (archive == 1), internalType: internalType, exists: out exists, newID: out id, errorMsg: out tempMsg);
                        if (exists)
                        {
                            saved = false;
                            tempMsg = string.Format("{0}{1}{2}", tempMsg, tempMsg.Length > 0 ? Environment.NewLine : "", "Cannot add duplicate Requirement Type record [" + RQMTType + "].");
                        }
                    }
                    else
                    {
                        saved = MasterData.RQMTType_Update(RQMTTypeID: id, RQMTType: RQMTType, description: description, sort: sortOrder, archive: (archive == 1), internalType: internalType, duplicate: out duplicate, errorMsg: out tempMsg);
                    }
                }
