...
public partial class MDGrid_WorkType_Organization : System.Web.UI.Page
{
    protected DataTable _dtWorkType = null;
    protected DataTable _dtOrganization = null;
    ...
    private void loadGridData()
    {
        ...
        _dtOrganization = UserManagement.LoadOrganizationList(includeArchive: false);
        DataTable dt = MasterData.WorkType_OrganizationList_Get(workTypeID: this._qfWorkTypeID);
    ...
    void grdMD_GridPageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        ...
        if (HttpContext.Current.Session["dtMD_WorkType_ORGANIZATION"] == null)
        {
            loadGridData();
        }
        else
        {
            grdMD.DataSource = (DataTable)HttpContext.Current.Session["dtMD_WorkType_ORGANIZATION"];
        }
    }
    ...
    void formatColumnDisplay(ref GridViewRow row)
    {
        ...
        row.Cells[DCC.IndexOf("ORGANIZATION")].Style["width"] = "150px";
    ...
    public static string SaveChanges(string rows)
    {
        ...
        try
        {
            ...
            int id = 0, organizationID = 0, workTypeID = 0, archive = 0;
            ...
            foreach (DataRow dr in dtjson.Rows)
            {
                ...
                int.TryParse(dr["WorkType_ORGANIZATIONID"].ToString(), out id);
                int.TryParse(dr["ORGANIZATION"].ToString(), out organizationID);
                int.TryParse(dr["WorkType"].ToString(), out workTypeID);
                int.TryParse(dr["ARCHIVE"].ToString(), out archive);

                if (organizationID == 0 || workTypeID == 0)
                {
                    tempMsg = "You must specify an Organization and a Work Type.";
                    saved = false;
                }
                else
                {
                    ...
                    {
                        exists = false;
                        saved = MasterData.WorkType_Organization_Add(organizationID: organizationID, workTypeID: workTypeID, exists: out exists, newID: out id, errorMsg: out tempMsg);
                        if (exists)
                        {
                            saved = false;
                            tempMsg = string.Format("{0}{1}{2}", tempMsg, tempMsg.Length > 0 ? Environment.NewLine : "", "Cannot add duplicate WorkType - Organization record.");
                        }
                    }
                    else
                    {
                        saved = MasterData.WorkType_Organization_Update(workTypeOrganizationID: id, organizationID: organizationID, workTypeID: workTypeID, archive: (archive == 1), errorMsg: out tempMsg);
                    }
                }
