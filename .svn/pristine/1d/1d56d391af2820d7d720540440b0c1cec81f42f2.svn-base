...
public partial class MDGrid_AOR_Type : System.Web.UI.Page
{
    ...
    private void exportExcel(DataTable dt)
    {
        ...
        String strName = "Master Grid - AOR Workload Type";
        ...
        {
            if (parentRow.Field<int>("AORWorkTypeID") != 0)
            {
    ...
    private static void formatParent(ref DataTable dt)
    {
        ...
        dt.Columns["AORWorkTypeName"].ColumnName = "AOR Workload Type";
    }
            ...
            foreach (DataColumn gridColumn in dt.Columns)
            {
                ...
                switch (gridColumn.ColumnName)
                {
                    ...
                    case "AORWorkTypeID":
                        blnVisible = false;
                        break;
                    case "AORWorkTypeName":
                        displayName = "AOR Workload Type";
                        blnVisible = true;
                        blnSortable = true;
                        break;
                    case "Description":
    ...
    void grdMD_GridRowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        //add edit link
        string itemId = row.Cells[DCC.IndexOf("AORWorkTypeID")].Text.Trim();
        if (itemId == "0")
        {
        ...
        if (CanView)
        {
            row.Cells[DCC.IndexOf("AORWorkTypeName")].Controls.Add(WTSUtility.CreateGridTextBox("AORWorkTypeName", itemId, Server.HtmlDecode(row.Cells[DCC.IndexOf("AORWorkTypeName")].Text.Replace("&nbsp;", " ").Trim())));
        ...
        //    row.Cells[DCC["A"].Ordinal].Controls.Add(imgBlank);
           ...
           row.Cells[DCC["X"].Ordinal].Controls.Add(WTSUtility.CreateGridDeleteButton(itemId, row.Cells[DCC.IndexOf("AORWorkTypeName")].Text.Trim()));
        //}
    ...
    void grdMD_GridPageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        ...
        if (HttpContext.Current.Session["dtMD_AOR_Type"] == null)
        {
            loadGridData();
        }
        else
        {
            grdMD.DataSource = (DataTable)HttpContext.Current.Session["dtMD_AOR_Type"];
        }
    }
    ...
    void formatColumnDisplay(ref GridViewRow row)
    {
        ...
        row.Cells[DCC.IndexOf("AORWorkTypeName")].Style["width"] = "150px";
    ...
    public static string SaveChanges(string rows)
    {
        ...
        try
        {
            ...
            string AORWorkTypeName = string.Empty, description = string.Empty;
            ...
            foreach (DataRow dr in dtjson.Rows)
            {
                id = 0;
                sortOrder = 0;
                AORWorkTypeName = string.Empty;
                description = string.Empty;
                archive = 0;

                tempMsg = string.Empty;
                int.TryParse(dr["AORWorkTypeID"].ToString(), out id);
                AORWorkTypeName = server.UrlDecode(dr["AORWorkTypeName"].ToString());
                ...
                if (string.IsNullOrWhiteSpace(AORWorkTypeName))
                {
                    tempMsg = "You must specify a value for AOR Workload Type.";
                    saved = false;
                }
                else
                {
                    ...
                    {
                        exists = false;
                        saved = MasterData.AORType_Add(AORWorkTypeName: AORWorkTypeName, description: description, sort: sortOrder, archive: (archive == 1), exists: out exists, newID: out id, errorMsg: out tempMsg);
                        if (exists)
                        {
                            saved = false;
                            tempMsg = string.Format("{0}{1}{2}", tempMsg, tempMsg.Length > 0 ? Environment.NewLine : "", "Cannot add duplicate AOR Workload Type record [" + AORWorkTypeName + "].");
                        }
                    }
                    else
                    {
                        saved = MasterData.AORType_Update(AORWorkTypeID: id, AORWorkTypeName: AORWorkTypeName, description: description, sort: sortOrder, archive: (archive == 1), duplicate: out duplicate, errorMsg: out tempMsg);
                    }
                }
    ...
    public static string DeleteItem(int itemId, string item)
    {
        ...
        try
        {
            ...
            {
                deleted = MasterData.AORType_Delete(itemId, out exists, out hasDependencies, out archived, out errorMsg);
