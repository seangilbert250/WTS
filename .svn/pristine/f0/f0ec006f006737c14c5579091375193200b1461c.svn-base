<%@ Page Title="" Language="C#" MasterPageFile="~/Grids.master" AutoEventWireup="true" CodeFile="MDGrid_ItemType_ResourceType.aspx.cs" Inherits="MDGrid_ItemType_ResourceType" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - Work Activity / Resource Type</asp:Content>
<asp:Content ID="cpHead" ContentPlaceHolderID="head" runat="Server"></asp:Content>
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">Work Activity / Resource Type (<span id="spanRowCount" runat="server">0</span>)</asp:Content>
	...
	<script id="jsAJAX" type="text/javascript">
		...
		function save_done(result) {
			try {
				...
				if (saved > 0) {
				    msg = 'Successfully saved Work Activity - Resource Types.' + saved;
					if (opener && opener.refreshPage) {
						opener.refreshPage();
					}
				}
				if (failed > 0) {
				    msg += '\n' + 'Failed to save Work Activity - Resource Types.' + failed;
				}
				MessageBox(msg);
		...
		function initVariables() {
			try {
				...
				if (_dcc[0] && _dcc[0].length > 0) {
					...
					_idxID = +'<%=this.DCC == null ? 0 : this.DCC["WorkActivity_WTS_RESOURCE_TYPEID"].Ordinal %>';
				}
			} catch (e) {
