<%@ Page Title="" Language="C#" MasterPageFile="~/Grids.master" AutoEventWireup="true" CodeFile="MDGrid_Contract.aspx.cs" Inherits="MDGrid_Contract" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - Contract Grid</asp:Content>
...
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">
    <table cellpadding="0" cellspacing="0" border="0" style="width: 100%">
        <tr>
            <td>
             Contract (<span id="spanRowCount" runat="server">0</span>)
            </td>
		...
		<tr>
            <td style="padding-left: 5px; display: none;">Child View:
                <asp:DropDownList ID="ddlChildView" runat="server" TabIndex="1" Style="width: 145px;">               
                    <asp:ListItem Text="Suite" Value="0" />
					<asp:ListItem Text="Narrative" Value="1" />
					<%--<asp:ListItem Text="Workload Allocation" Value="2" />--%>
				</asp:DropDownList>
            </td>
			<td style="padding-left: 5px;">Contract Type:
...
<asp:Content ID="cpBody" ContentPlaceHolderID="ContentPlaceHolderBody" runat="Server">
	...
	<script id="jsEvents" type="text/javascript">
		...
		function refreshPage() {
			...
			qs = editQueryStringValue(qs, 'ContractID', $('#<%=this.ddlQF.ClientID %> option:selected').val());
		...
		function lbEdit_click(itemID) {
			if (parent.ShowFrameForEdit) {
				parent.ShowFrameForEdit(_pageURLs.MasterData.MDType.Contract, false, itemID, true);
			}
			else {
				var title = '', url = '';
				var h = 700, w = 1000;

				title = 'Contract - [' + itemID + ']';
				url = _pageURLs.MasterData.Edit.Contract
					+ '?itemID=' + itemID;

				//open in a popup
				var openPopup = popupManager.AddPopupWindow('Contract', title, 'Loading.aspx?Page=' + url, h, w, 'PopupWindow', this);
				if (openPopup) {
					openPopup.Open();
				}
			}
		        ...
		        if (direction.toUpperCase() == "SHOW") {
		            ...
		            $(sender).closest('tr').each(function () {
		                ...
		                td = $('td:eq(<%=(this.DCC == null 
							|| !this.DCC.Contains("ContractType")) ? 0 : this.DCC["ContractType"].Ordinal %>)', row)[0];
						loadChildGrid(td, id);
					});
                }
                else {
        ...
        function loadChildGrid(td, id) {
            ...
            switch ($('#<%=this.ddlChildView.ClientID %> option:selected').val()) {
                case "0": // Suite
                    url += 'MDGrid_Contract_System.aspx';
                    url += '?ContractID=' + id;
                    break;
                case "1": // Narrative
                    url += 'MDGrid_Narrative_CONTRACT.aspx';
                    url += '?ContractID=' + id;
                    break;
                case "2": // Workload Allocation
                    url += 'MDGrid_WorkloadAllocation_CONTRACT.aspx';
                    url += '?ContractID=' + id;
                    break;
                default:
                    break;
		...
		function initVariables() {
			try {
				...
				if (_dcc[0] && _dcc[0].length > 0) {
					...
					_idxID = parseInt('<%=this.DCC["ContractID"].Ordinal %>');
					_idxName = parseInt('<%=this.DCC["Contract"].Ordinal %>');
