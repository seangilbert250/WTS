...
public partial class MDGrid_ItemType_Status : System.Web.UI.Page
{
    ...
    private void loadGridData()
	{
        ...
        _dtStatus.DefaultView.RowFilter = "StatusTypeID IN (0, 1)";
        _dtStatus.DefaultView.Sort = "StatusID ASC";
        _dtStatus = _dtStatus.DefaultView.ToTable();

        DataTable dt = MasterData.WORKITEMTYPE_StatusList_Get(WORKITEMTYPEID: this._qfWorkItemTypeID, STATUSID: this._qfStatusID);
    ...
    void grdMD_GridPageIndexChanging(object sender, GridViewPageEventArgs e)
	{
		...
		if (HttpContext.Current.Session["dtMD_WORKITEMTYPE_Status"] == null)
		{
			loadGridData();
		}
		else
		{
			grdMD.DataSource = (DataTable)HttpContext.Current.Session["dtMD_WORKITEMTYPE_Status"];
		}
	}
    ...
    void formatColumnDisplay(ref GridViewRow row)
	{
		...
		row.Cells[DCC.IndexOf("STATUS")].Style["width"] = "225px";
	...
	public static string SaveChanges(string workItemType_ID, string rows)
	{
        ...
        try
        {
            ...
            else
            {
                    ...
                    int id = 0, workItemTypeID = 0, statusID = 0, archive = 0;  //, ALLOCATIONGROUPID = 0;
                    ...
                    foreach (DataRow dr in dtjson.Rows)
                    {
                        ...
                        int.TryParse(dr["WORKITEMTYPE_StatusID"].ToString(), out id);
                        ...
                        if (statusID == 0)
                        {
                            tempMsg = "You must specify a Status.";
                            saved = false;
                        }
                        else
                        {
                            ...
                            {
                                exists = false;
                            saved = MasterData.WORKITEMTYPE_Status_Add(WORKITEMTYPEID: workItemTypeID
                                    , STATUSID: statusID
                                    , exists: out exists
                                    , newID: out id
                                ...
                                {
                                    saved = false;
                                    tempMsg = string.Format("{0}{1}{2}", tempMsg, tempMsg.Length > 0 ? Environment.NewLine : "", "Cannot add duplicate Work Activity - Status record.");
                                }
                            }
                            else
                            {
                                saved = MasterData.WORKITEMTYPE_Status_Update(WORKITEMTYPE_StatusID: id
	...
	public static string DeleteItem(int itemId, string item)
	{
		...
		try
		{
			...
			{
				deleted = MasterData.WORKITEMTYPE_Status_Delete(WORKITEMTYPE_StatusID: itemId, exists: out exists, errorMsg: out errorMsg);
			}
		}
		catch (Exception ex)
		{
