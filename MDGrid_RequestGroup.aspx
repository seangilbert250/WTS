<%@ Page Title="" Language="C#" MasterPageFile="~/Grids.master" AutoEventWireup="true" CodeFile="MDGrid_RequestGroup.aspx.cs" Inherits="MDGrid_RequestGroup" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - Request Group Grid</asp:Content>
...
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">
        ...
        <tr>
            <td>
                Request Group (<span id="spanRowCount" runat="server">0</span>)
            </td>
...
<asp:Content ID="cpGridSetting" ContentPlaceHolderID="cphGridSettings" runat="Server">
	<table id="tableQuickFilters" cellpadding="0" cellspacing="0">
		<tr>
			<td style="padding-left: 5px;">Request Group:
				<asp:DropDownList ID="ddlQF" runat="server" TabIndex="1" AppendDataBoundItems="true" Style="width: 400px;">
	...
	<script id="jsEvents" type="text/javascript">
		...
		function lbEdit_click(itemID) {
			if (parent.ShowFrameForEdit) {
				parent.ShowFrameForEdit(_pageURLs.MasterData.MDType.RequestGroup, false, itemID, true);
			}
			else {
				var title = '', url = '';
				var h = 700, w = 1000;

				title = 'RequestGroup - [' + itemID + ']';
				url = _pageURLs.MasterData.Edit.RequestGroup
					+ '?itemID=' + itemID;

				//open in a popup
				var openPopup = popupManager.AddPopupWindow('RequestGroup', title, 'Loading.aspx?Page=' + url, h, w, 'PopupWindow', this);
				if (openPopup) {
					openPopup.Open();
				}
			}
		...
		function initVariables() {
			try {
				...
				if (_dcc[0] && _dcc[0].length > 0) {
					...
					_idxID = parseInt('<%=this.DCC["RequestGroupID"].Ordinal %>');
					_idxName = parseInt('<%=this.DCC["RequestGroup"].Ordinal %>');
