<%@ Page Title="" Language="C#" MasterPageFile="~/Grids.master" AutoEventWireup="true" CodeFile="MDGrid_WorkType.aspx.cs" Inherits="MDGrid_WorkType" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - Resource Group Grid</asp:Content>
...
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">
        ...
        <tr>
            <td>
                 Resource Group (<span id="spanRowCount" runat="server">0</span>)
            </td>
...
<asp:Content ID="cpPageContentInfo" ContentPlaceHolderID="cphPageContentInfo" runat="Server">
	<table cellpadding="0" cellspacing="0" style="float: right; padding-right: 2px;">
		<tr>
			<td>
				<input type="button" id="buttonSync" value="Sync Resources" disabled="disabled" />
...
<asp:Content ID="cpBody" ContentPlaceHolderID="ContentPlaceHolderBody" runat="Server">
	...
	<div id="divSaving" style="position: absolute; left: 35%; top: 15%; padding: 10px; background: white; border: 1px solid grey; font-size: 18px; text-align: center; display: none;">
		<table>
			...
			<tr>
				<td>
					<!-- <img alt='' src="Images/loaders/WorkType_bar_blue.gif" /> -->
				</td>
			</tr>
		</table>
	</div>
	...
	<script id="jsAJAX" type="text/javascript">
		...
		function deleteItem(itemId, item) {
			try {
				...
				ShowDimmer(false);
				MessageBox('There was an error gathering data to update.\n' + e.message);
			}
		}
	...
	<script id="jsEvents" type="text/javascript">
		...
		function refreshPage() {
			...
			qs = editQueryStringValue(qs, 'WorkTypeID', $('#<%=this.ddlQF.ClientID %> option:selected').val());
		...
		function lbEditPhases_click(itemID) {
			var title = '', url = '';
			var h = 500, w = 800;

			title = 'Work Type - Phases';
			url = 'MDGrid_WorkType_Phase.aspx?WorkTypeID=' + itemID;

			//open in a popup
			var openPopup = popupManager.AddPopupWindow('WorkType_Phases', title, 'Loading.aspx?Page=' + url, h, w, 'PopupWindow', this);
			if (openPopup) {
				openPopup.Open();
			}
		}
		
		function lbEditStatuses_click(itemID) {
			var title = '', url = '';
			var h = 500, w = 800;

			title = 'Resource Group - Statuses';
			url = 'MDGrid_WorkType_Status.aspx?WorkTypeID=' + itemID;

			//open in a popup
			var openPopup = popupManager.AddPopupWindow('WorkType_Statuses', title, 'Loading.aspx?Page=' + url, h, w, 'PopupWindow', this);
			if (openPopup) {
				openPopup.Open();
			}
        }

        function lbEditResourceTypes_click(itemID) {
            var title = '', url = '';
            var h = 500, w = 800;

            title = 'Resource Group - Resource Types';
            url = 'MDGrid_WorkType_ResourceType.aspx?WorkTypeID=' + itemID;

            //open in a popup
            var openPopup = popupManager.AddPopupWindow('WorkType_ResourceTypes', title, 'Loading.aspx?Page=' + url, h, w, 'PopupWindow', this);
            if (openPopup) {
                openPopup.Open();
            }
        }

        function lbEditOrganizations_click(itemID) {
            var title = '', url = '';
            var h = 500, w = 800;

            title = 'Resource Group - Organizations';
            url = 'MDGrid_WorkType_Organization.aspx?WorkTypeID=' + itemID;

            //open in a popup
            var openPopup = popupManager.AddPopupWindow('WorkType_Organizations', title, 'Loading.aspx?Page=' + url, h, w, 'PopupWindow', this);
            if (openPopup) {
                openPopup.Open();
            }
        }
        ...
        function buttonSync_click() {
            if (confirm('This will sync Resources to Resource Groups based on the associated Resource Types and Organizations.' + '\n' + 'Do you wish to continue?')) {
                syncResources();
            }
        }
        ...
        function syncResources() {
            ...
            PageMethods.SaveResource(save_done, on_error);
        }
				...
				if (direction.toUpperCase() == "SHOW") {
					...
					$(sender).closest('tr').each(function () {
						...
						td = $('td:eq(<%=(this.DCC == null 
							|| !this.DCC.Contains("WorkType")) ? 0 : this.DCC["WorkType"].Ordinal %>)', row)[0];
						loadChildGrid(td, id);
					});
				}
				else {
		...
		function loadChildGrid(td, id) {
            ...
            url += 'MDGrid_WorkType_Resource.aspx';
            url += '?WorkTypeID=' + id;
		...
		function initVariables() {
			try {
				...
				if (_dcc[0] && _dcc[0].length > 0) {
					...
					_idxID = parseInt('<%=this.DCC["WorkTypeID"].Ordinal %>');
					_idxName = parseInt('<%=this.DCC["WorkType"].Ordinal %>');
