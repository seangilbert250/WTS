...
public partial class MDGrid_ItemType_ResourceType : System.Web.UI.Page
{
    ...
    private void loadGridData()
	{
        ...
        DataTable dt = MasterData.WorkActivity_WTS_RESOURCE_TYPEList_Get(WORKITEMTYPEID: this._qfWorkItemTypeID);
    ...
    void grdMD_GridRowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		//add edit link
		string itemId = row.Cells[DCC.IndexOf("WorkActivity_WTS_RESOURCE_TYPEID")].Text.Trim();
		if (itemId == "0")
		{
		...
		if (CanView)
		{
			string selectedId = row.Cells[DCC.IndexOf("WTS_RESOURCE_TYPEID")].Text;
			string selectedText = row.Cells[DCC.IndexOf("WTS_RESOURCE_TYPE")].Text;
		...
		{
			row.Cells[DCC["X"].Ordinal].Controls.Add(WTSUtility.CreateGridDeleteButton(itemId, row.Cells[DCC.IndexOf("WorkActivity_WTS_RESOURCE_TYPEID")].Text.Trim()));
		}
	}
    ...
    void grdMD_GridPageIndexChanging(object sender, GridViewPageEventArgs e)
	{
		...
		if (HttpContext.Current.Session["dtMD_WORKITEMTYPE_WTS_RESOURCE_TYPE"] == null)
		{
			loadGridData();
		}
		else
		{
			grdMD.DataSource = (DataTable)HttpContext.Current.Session["dtMD_WORKITEMTYPE_WTS_RESOURCE_TYPE"];
		}
	}
    ...
    void formatColumnDisplay(ref GridViewRow row)
	{
		...
		row.Cells[DCC.IndexOf("WTS_RESOURCE_TYPE")].Style["width"] = "225px";
	...
	public static string DeleteItem(int itemId, string item)
	{
		...
		try
		{
			...
			{
				deleted = MasterData.WORKITEMTYPE_ResourceType_Delete(WorkActivity_WTS_RESOURCE_TYPEID: itemId, exists: out exists, errorMsg: out errorMsg);
			}
		}
		catch (Exception ex)
		{
