<%@ Page Title="" Language="C#" MasterPageFile="~/Grids.master" AutoEventWireup="true" CodeFile="MDGrid_ItemType_Status.aspx.cs" Inherits="MDGrid_ItemType_Status" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - Work Activity / Status</asp:Content>
<asp:Content ID="cpHead" ContentPlaceHolderID="head" runat="Server"></asp:Content>
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">Work Activity / Status (<span id="spanRowCount" runat="server">0</span>)</asp:Content>
	...
	<script id="jsAJAX" type="text/javascript">
			...
			try {
				...
				$('.gridBody, .selectedRow', $('#<%=this.grdMD.ClientID%>_Grid')).each(function (i, row) {
					...
					if (_dcc[0].length > 0 && $(this)[0].hasAttribute('fieldChanged')) {
						...
						if (changed) {
							for (var i = 0; i <= _dcc[0].length - 1; i++) {
								changedRow.push('"' + _dcc[0][i].ColumnName + '":"' + (_dcc[0][i].ColumnName == 'STATUS' ? encodeURIComponent(GetColumnValue(row, i)) : GetColumnValue(row, i)) + '"');
							}
		...
		function save_done(result) {
			try {
				...
				if (saved > 0) {
				    msg = 'Successfully saved Work Activity - Statuses.' + saved;
					if (opener && opener.refreshPage) {
						opener.refreshPage();
					}
				}
				if (failed > 0) {
				    msg += '\n' + 'Failed to save Work Activity - Statuses.' + failed;
				}
				MessageBox(msg);
		...
		function initVariables() {
			try {
				...
				if (_dcc[0] && _dcc[0].length > 0) {
					...
					_idxID = +'<%=this.DCC == null ? 0 : this.DCC["WorkItemType_StatusID"].Ordinal %>';
				}
			} catch (e) {
