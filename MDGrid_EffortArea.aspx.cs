...
public partial class MDGrid_EffortArea : System.Web.UI.Page
{
    ...
    private void exportExcel(DataTable dt)
    {
        ...
        String strName = "Master Grid - Area/Size of Effort";
        ...
        {
            if (parentRow.Field<int>("EffortAreaID") != 0)
            {
    ...
    private void printChildRows(DataRow parentRow, Worksheet ws, ref int rowCount)
    {
        ...
        int ID = parentRow.Field<int>("EffortAreaID");
    ...
    private static void formatParent(ref DataTable dt)
    {
        ...
        dt.Columns["EffortArea"].ColumnName = "Area of Effort";
    }
    private void loadGridData()
	{
		DataTable dt;
		
		if(_refreshData || Session["dtMD_EffortArea"] == null){
			dt = MasterData.EffortAreaList_Get(includeArchive: true);
			HttpContext.Current.Session["dtMD_EffortArea"] = dt;
		}
		else
		{
			dt = (DataTable)HttpContext.Current.Session["dtMD_EffortArea"];
		}
	...
	void grdMD_GridRowDataBound(object sender, GridViewRowEventArgs e)
	{
		...
		//add edit link
		string itemId = row.Cells[DCC.IndexOf("EffortAreaID")].Text.Trim();
		if (itemId == "0")
		{
		...
		if (CanView)
		{
			row.Cells[DCC.IndexOf("EffortArea")].Controls.Add(WTSUtility.CreateGridTextBox("EffortArea", itemId, Server.HtmlDecode(row.Cells[DCC.IndexOf("EffortArea")].Text.Replace("&nbsp;", " ").Trim())));
	...
	void grdMD_GridPageIndexChanging(object sender, GridViewPageEventArgs e)
	{
		...
		if (HttpContext.Current.Session["dtMD_EffortArea"] == null)
		{
			loadGridData();
		}
		else
		{
			grdMD.DataSource = (DataTable)HttpContext.Current.Session["dtMD_EffortArea"];
		}
	}
	...
	public static string DeleteItem(int itemId, string item)
	{
		...
		try
		{
			...
			{
				deleted = MasterData.EffortArea_Delete(itemId, out exists, out hasDependencies, out archived, out errorMsg);
