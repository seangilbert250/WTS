<%@ Page Title="" Language="C#" MasterPageFile="~/Grids.master" AutoEventWireup="true" CodeFile="MDGrid_WorkType_Resource.aspx.cs" Inherits="MDGrid_WorkType_Resource" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - Resource Group/Resource Grid</asp:Content>
<asp:Content ID="cpHead" ContentPlaceHolderID="head" runat="Server"></asp:Content>
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">Resource Group/Resource (<span id="spanRowCount" runat="server">0</span>)</asp:Content>
...
<asp:Content ID="cpBody" ContentPlaceHolderID="ContentPlaceHolderBody" runat="Server">
	...
	<script id="jsAJAX" type="text/javascript">
		...
		function save() {
			try {
				...
				if (changedRows.length == 0) {
					...
					PageMethods.SaveChanges(<%=_WorkTypeID%>, json, save_done, on_error);
				}
			} catch (e) {
				ShowDimmer(false);
        ...
        function confirmResourceRemove(answer) {
            if ($.trim(answer).toUpperCase() == 'YES') {
                try {
                    ...
                    $('input[type="checkbox"]:checked').each(function () {
                        ...
                        arrResource.push({ 'worktype_resource_id': $obj.attr('worktype_resource_id') });
                    });

                    ShowDimmer(true, 'Removing...', 1);

                    var nJSON = JSON.stringify(arrResource);

                    PageMethods.DeleteItems(nJSON, function (result) { delete_done(result, arrResource.length); }, on_error);
                }
                catch (e) {
                    ShowDimmer(false);
        ...
        function delete_done(result, checkedCount) {
            ...
            if (blnDeleted) {
                MessageBox((checkedCount > 1 ? 'Resources have' : 'Resource has') + ' been removed from the Resource Group.');
                refreshPage(true);
            }
            else {
                MessageBox('Failed to remove. <br>' + errorMsg);
            }
        }
	...
	<script id="jsEvents" type="text/javascript">
        ...
        function buttonDelete_click() {
            ...
            if (checkedCount > 0) {
                QuestionBox('Confirm Resource Removal', 'Are you sure you want to remove the checked Resource' + (checkedCount > 1 ? 's' : '') + ' from the Resource Group?', 'Yes,No', 'confirmResourceRemove', 300, 300, this);
            } else {
		...
		function initVariables() {
			try {
				...
				if (_dcc[0] && _dcc[0].length > 0) {
					...
					_idxID = +'<%=this.DCC == null ? 0 : this.DCC["WorkType_WTS_RESOURCEID"].Ordinal %>';
				}
			} catch (e) {
