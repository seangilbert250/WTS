<%@ Page Title="" Language="C#" MasterPageFile="~/Grids.master" AutoEventWireup="true" CodeFile="MDGrid_PDD_TDR_Phase.aspx.cs" Inherits="MDGrid_PDD_TDR_Phase" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - PD2TDR Phase Grid</asp:Content>
...
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">
        ...
        <tr>
            <td>
                PD2TDR Phase (<span id="spanRowCount" runat="server">0</span>)
            </td>
...
<asp:Content ID="cpGridSetting" ContentPlaceHolderID="cphGridSettings" runat="Server">
	<table id="tableQuickFilters" cellpadding="0" cellspacing="0" style="display:none;">
		<tr>
			<td style="padding-left: 5px;">PD2TDR Phase:
...
<asp:Content ID="cpBody" ContentPlaceHolderID="ContentPlaceHolderBody" runat="Server">
	...
	<script id="jsEvents" type="text/javascript">
		...
		function refreshPage() {
			...
			qs = editQueryStringValue(qs, 'PhaseID', $('#<%=this.ddlQF.ClientID %> option:selected').val());
		...
		function lbEditWorkTypes_click(itemID) {
			var title = '', url = '';
			var h = 500, w = 800;

            title = 'Work Activity - PD2TDR Phases';
			url = 'MDGrid_WorkType_Phase.aspx?PhaseID=' + itemID;
		...
		function initVariables() {
			try {
				...
				if (_dcc[0] && _dcc[0].length > 0) {
					...
					_idxID = parseInt('<%=this.DCC["PDDTDR_PhaseID"].Ordinal %>');
					_idxName = parseInt('<%=this.DCC["PDDTDR_Phase"].Ordinal %>');
