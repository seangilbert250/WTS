<%@ Page Title="" Language="C#" MasterPageFile="~/EditTabs.master" AutoEventWireup="true" CodeFile="UserProfile_Certifications.aspx.cs" Inherits="Admin_UserProfile_Certifications" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">User Certifications</asp:Content>
...
<asp:Content ID="cpHeaderImage" ContentPlaceHolderID="ContentPlaceHolderHeaderImage" runat="Server">
	<img src="../Images/Icons/pencil.png" alt="Review/Edit User Cerfifications" width="15" height="15" style="cursor: default;" />
</asp:Content>
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeaderText" runat="Server">User Certifications (<span id="spanRowCount" runat="server">0</span>)</asp:Content>
...
<asp:Content ID="cpBody" ContentPlaceHolderID="ContentPlaceHolderBody" runat="Server">
	...
	<asp:GridView runat="server" ID="grdCerts" AllowPaging="false" CssClass="grid" RowStyle-CssClass="gridBody" SelectedRowStyle-CssClass="selectedRow" HeaderStyle-CssClass="gridHeader" AlternatingRowStyle-BackColor="#dfdfdf" style="width:100%;"></asp:GridView>
	...
	<script id="jsVariables" type="text/javascript">
		...
		var _userID = 0;
		var _idxDelete = 0, _idxID = 0, _idxName = 0, _idxDescription = 0, _idxExpirationDate = 0, _idxExpired = 0;
	...
	<script id="jsAJAX" type="text/javascript">
		...
		function save() {
			try {
				...
				$('.gridBody, .selectedRow', $('#<%=this.grdCerts.ClientID%>')).each(function (i, row) {
					var changedRow = [];
					var changed = false;
				...
				if (changedRows.length == 0) {
					MessageBox('You have not made any changes', 'Invalid Input');
				}
				else {
	...
	<script id="jsEvents" type="text/javascript">
		...
		function buttonNew_click() {
			var grdCerts = <%=this.grdCerts.ClientID%>;
			
			if (grdCerts.rows.length == 0) {
				return;
			}
			var nRow = grdCerts.rows[grdCerts.rows.length - 1].cloneNode(true);
			...
			$(nRow.cells).each(function(i, td) {
				if($(td).find('input:text').length > 0) {
					...
					if($(td).children(0).attr('id').indexOf('txtExpiration') >= 0) {
						$(td).children(0).datepicker({
							changeMonth: true
			...
			grdCerts.rows[0].parentNode.insertBefore(nRow,grdCerts.rows[1]);
			//add delete button
		...
		function initVariables() {
			try {
				...
				if (_dcc[0] && _dcc[0].length > 0) {
					...
					_idxID = +'<%=this.DCC == null ? 0 : this.DCC["Resource_CertificationID"].Ordinal %>';
					_idxName = +'<%=this.DCC == null ? 0 : this.DCC["Resource_Certification"].Ordinal %>';
					_idxDescription = +'<%=this.DCC == null ? 0 : this.DCC["Description"].Ordinal %>';
					_idxExpirationDate = +'<%=this.DCC == null ? 0 : this.DCC["Expiration_Date"].Ordinal %>';
					_idxExpired = +'<%=this.DCC == null ? 0 : this.DCC["Expired"].Ordinal %>';
				}
			} catch (e) {
