<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MDGrid_Contract_System.aspx.cs" Inherits="MDGrid_Contract_System" MasterPageFile="~/Grids.master" Theme="Default" %>

<asp:Content ID="cpHeadTitle" ContentPlaceHolderID="headTitle" runat="Server">Master Data - Contract / Systems Grid</asp:Content>
<asp:Content ID="cpHead" ContentPlaceHolderID="head" runat="Server"></asp:Content>
<asp:Content ID="cpHeaderText" ContentPlaceHolderID="ContentPlaceHolderHeader" runat="Server">Contract / Systems (<span id="spanRowCount" runat="server">0</span>)</asp:Content>
...
<asp:Content ID="cpBody" ContentPlaceHolderID="ContentPlaceHolderBody" runat="Server">
		    ...
		    try {
		        ...
		        if (validation.length == 0) {
		            ...
		            if (changedRows.length == 0) {
		                ...
		                PageMethods.SaveChanges('<%=_qfContractID %>', json, save_done, on_error);
		            }
		        }
		        else {
	...
	<script id="jsEvents" type="text/javascript">
		...
		function buttonNew_click() {
			...
			$(nRow.cells).each(function(i, td){
				if($(td).find('input:text').length > 0) {
					...
					if (_dcc[0][i].ColumnName.toUpperCase() == 'SYSTEM') $(td).find('select').attr('field', 'System');
				}
				else{
					$(td).html('&nbsp;');
				}
        ...
        function validate() {
		    var validation = [];
		    var $systemRows = $('.gridBody:visible, .selectedRow:visible', $('#<%=this.grdMD.ClientID %>_Grid'));
            var blnExit = false;

            $.each($systemRows, function () {
			    if ($(this).find('select[field="System"]').val() == '0') validation.push('System cannot be empty.');
			    return false;
			});
